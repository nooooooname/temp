...
| xp /nuf 地址 | 查看某一物理地址的内存值，x和xp命令中nuf的含义如下：<br>n 打印的单元数<br>u 每个单元的大小，u可以是下列之一：<br>b 字节<br>h 字<br>w 双字<br><br>f 打印格式，f可以是下列之一：<br>x 十六进制<br>d 十进制<br>u 无符号十进制<br>o 八进制<br>t 二进制<br>c 字符<br><br>n、u、f是可选参数，如果不指定，则n默认为1，u默认是w，f默认是x。<br>如果前面使用过x或者xp命令，则u和f默认等于上一次x或xp命令所使用的值。<br>只要指定了n、u、f中的任意一个，前边的“/”就不能省略。<br>此外，地址也是一个可选参数，如果不指定，则默认是0，<br>如过前面使用过x或者xp命令，则地址默认等于上一次的地址+n×u。 |
